...
namespace Serial {
	struct Packet {
		...
		Packet& operator<<(float d) { data.push_back(((long)f & 0xff000000) >> 24); data.push_back(((long)f & 0xff0000) >> 16); data.push_back(((long)f & 0xff00) >> 8); data.push_back(((long)f & 0xff)); return *this; }
		...
		Packet& operator>>(float d) { d = data[cur] + (data[cur + 1] << 8) + (data[cur + 2] << 16) + (data[cur + 3] << 24); cur += 4; return *this; }
